...
- O journal loopback é propriedade exclusiva do servidor confiável. Cada medição abre uma janela server-owned e percorre `OPEN → QUIESCING → SEALED → VERIFIED/REJECTED`; a quiescência exige amostras estáveis, o selo fixa cardinalidade e digest, e qualquer evento ou registo posterior invalida o resultado. A decisão compara o conjunto completo por identidade server-owned transportada na resposta: cada pedido e resposta observados liga-se a exatamente um registo, e cada registo liga-se a exatamente uma resposta observada. Pedidos internos do WebKit que não carreguem a capacidade efémera injetada pelo consumidor são recusados com zero bytes antes de entrarem no journal autoritativo; nunca são promovidos por semelhança de rota. Extra, desconhecido, duplicado, incompleto ou divergente falha fechado.
