...
- O contexto medido é encerrado antes dos probes. Cada probe usa contexto próprio, identidade derivada de base/head/payload/engine/ação e exatamente uma observação. A autoridade de observação permanece exclusivamente no processo confiável: `BrowserContext.route` e `routeWebSocket` bloqueiam antes da saída e produzem o registo; nenhum binding, token, envelope ou função global atravessa o realm da página candidata. O consumidor compara mecanismo, URL/origem, ação, fase, rota, viewport, engine, disposição e identidade com a expectativa canónica; wrappers, supressão, replay e transplante no conteúdo não conseguem fabricar a observação aceita.
- O controlo `consent-loader` executa a página viva corrigida `crm-gestao.html`: mede zero tentativa sem decisão, após recusa e após retirada; somente a aceitação válida produz a tentativa canónica do Meta Pixel, que continua bloqueada e classificada como achado pelo processo confiável. Uma nova retirada e recarga deve permanecer sem tentativa adicional.
- Toda requisição loopback é validada contra a allowlist de blobs antes de continuar. Caminho desconhecido, ambíguo, ausente, não pinado ou resposta local diferente de 200/206 cria uma violação operacional e reprova o relatório.
- O journal loopback é propriedade exclusiva do servidor confiável. Cada medição abre uma janela server-owned e percorre `OPEN → QUIESCING → SEALED → VERIFIED/REJECTED`; a quiescência exige amostras estáveis, o selo fixa cardinalidade e digest, e qualquer evento ou registo posterior invalida o resultado. Cada pedido autorizado liga-se a exatamente um registo completo do servidor; uma resposta observável do browser, quando existe, é apenas corroborante e precisa coincidir com esse registo. A ausência do evento do browser não cria sucesso por si só: exige o único registo completo 200/206 com a identidade consumer-owned exata. `status 0` permanece observação indeterminada, é contado e nunca promovido a resposta confiável. Pedidos internos sem headers Playwright são recusados com zero bytes e recebem uma identidade de recusa server-owned; a recusa observada precisa coincidir com ela. Extra, desconhecido, duplicado, incompleto ou divergente falha fechado.
- `preconnect` e `dns-prefetch` externos estáticos são recusados pelo servidor antes de a página ser entregue. A análise estrutural cobre atributos HTML cotados ou não cotados, ordem, espaçamento e capitalização, e falha fechada para sintaxe ambígua. Hints criados dinamicamente são detetados após inserção e tornam o ensaio inválido, independentemente de domínio ou URL; esta observação não comprova bloqueio pré-egress e permanece `NOT_VERIFIED`. Consentimento não converte o achado em PASS.
