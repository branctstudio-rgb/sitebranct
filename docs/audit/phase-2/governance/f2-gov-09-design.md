...
- O journal loopback é propriedade exclusiva do servidor confiável. Cada medição abre uma janela server-owned e percorre `OPEN → QUIESCING → SEALED → VERIFIED/REJECTED`; a quiescência exige amostras estáveis, o selo fixa cardinalidade e digest, e qualquer evento ou registo posterior invalida o resultado. A decisão compara o conjunto completo: pedidos/respostas/falhas observados devem possuir vínculo único, todos os registos têm identidade e janela server-owned, e registos internos sem header só são admitidos quando pertencem ao recurso de uma falha WebKit conhecida e integralmente comprovada. Extra, desconhecido, duplicado, incompleto ou divergente falha fechado.
