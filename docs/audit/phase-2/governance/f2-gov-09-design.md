...
- O contexto medido é encerrado antes dos probes. Cada probe usa contexto próprio, identidade derivada de base/head/payload/engine/ação e exatamente uma observação; conteúdo atrasado nunca compartilha o canal de controlo.
...
- `preconnect` e `dns-prefetch` externos estáticos são recusados pelo servidor antes de a página ser entregue. Hints criados dinamicamente são bloqueados e registrados pelo consumidor, independentemente de domínio ou URL; consentimento não converte tentativa bloqueada em PASS.
