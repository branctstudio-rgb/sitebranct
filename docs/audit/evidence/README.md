...
Reprodução independente: `node tests/audit/collect-browser-baseline.mjs --check fixtures/audit/baseline-results.json`. O coletor inicia apenas um servidor estático local e Chrome/Chromium headless, mede as 12 rotas nos cinco viewports e compara overflow, largura, targets, H1, `alt`, hreflang e consola com o snapshot. Para atualizar deliberadamente a prova: `node tests/audit/collect-browser-baseline.mjs fixtures/audit/baseline-results.json`.

- Home: 1440×900, 1024×768, 768×1024, 390×844 e 360×800.
- CRM: desktop, tablet e mobile.
- Website Premium: desktop, tablet e mobile.

As capturas de página inteira não têm necessariamente a largura solicitada: a barra de scroll reduz a área útil e, em mobile, o overflow horizontal expande a imagem até 416 px. As duas capturas com sufixo `viewport` registam apenas a área visível (1425×891 e 375×812). Isto é evidência do defeito observado, não uma equivalência entre tamanho solicitado e tamanho do ficheiro.
