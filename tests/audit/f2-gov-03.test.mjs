...
test("protected gate changes require a bounded human ceremony", async () => {
  ...
  const valid = {
    ...
    humanApproval:{ actor:"COUNCIL_DECISION_REQUIRED", headSha:"a".repeat(40), baseSha:expectedBase },
  ...
  assert.equal(validateGateEvolutionRequest(valid), true);
  for (const [label, mutate, expected] of [
    ...
    const copy = structuredClone(valid);
    mutate(copy);
    assert.throws(() => validateGateEvolutionRequest(copy), (error) => error.message.includes(expected));
  });
});
