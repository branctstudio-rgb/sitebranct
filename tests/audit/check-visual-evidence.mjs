...
const server = createServer(async (req,res)=>{try{const rel=decodeURIComponent(new URL(req.url,"http://local").pathname).replace(/^\/+/,"");const file=normalize(join(root,rel));assert.ok(file.startsWith(root));assert.ok((await stat(file)).isFile());res.writeHead(200,{"content-type":"image/jpeg","access-control-allow-origin":"*"});res.end(await readFile(file));}catch{res.writeHead(404);res.end();}}).listen(0,"127.0.0.1");
...
for(const item of manifest.files){
  const url=`http://127.0.0.1:${server.address().port}/docs/audit/evidence/baseline/${item.file}`;
  ...
  console.log(`${item.file}: ${(result.value.uniformBandRatio*100).toFixed(1)}% uniform, ${(result.value.lowDetailBandRatio*100).toFixed(1)}% low-detail bands`);
  if(result.value.uniformBandRatio>0.45) failures.push(`${item.file}: ${(result.value.uniformBandRatio*100).toFixed(1)}% uniform bands`);
}
