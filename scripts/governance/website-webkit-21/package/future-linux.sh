...
set -euo pipefail
[[ "${1:-}" == "--human-authorized-webkit-21" ]] || { echo 'NOT_EXECUTED: human authorization required'; exit 64; }
stage="${2:-}"
...
root='/var/tmp/branct-website-webkit-21'
...
if [[ "$stage" == prepare ]]; then
 ...
 docker run --name website-21-npm --platform linux/amd64 --user "$(id -u):$(id -g)" --cap-drop=ALL --security-opt=no-new-privileges --memory=2g --cpus=2 --pids-limit=256 --read-only --tmpfs /tmp:rw,nosuid,nodev,size=512m \
 ...
 docker run --name website-21-measure --pull=never --platform linux/amd64 --network=none --init --user "$(id -u):$(id -g)" \
  ...
  --mount "type=bind,src=$root/outputs,dst=/outputs" --workdir /candidate --env HOME=/tmp --env PLAYWRIGHT_BROWSERS_PATH=/ms-playwright --env F2_01_BROWSER=webkit \
